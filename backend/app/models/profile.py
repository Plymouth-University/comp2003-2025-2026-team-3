...
from sqlalchemy import Column, String, Text, Boolean, SmallInteger, ForeignKey, Index
...
class Tenant(ProfileBase):
    ...
    specialisms = relationship("Specialism", back_populates="tenant", cascade="all, delete-orphan")
    avatar_presets = relationship("AvatarPreset", back_populates="tenant", cascade="all, delete-orphan")
...
class Profile(ProfileBase):
    ...
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenant.tenant_id", ondelete="CASCADE"), nullable=False)
    status = Column(Text, nullable=False, default="active")  # active, deactivated, suspended
    created_at = Column(TIMESTAMP(timezone=True), server_default=func.now(), nullable=False)
    ...
    display = relationship("ProfileDisplay", back_populates="profile", uselist=False, cascade="all, delete-orphan")
    identities = relationship("ProfileIdentity", back_populates="profile", cascade="all, delete-orphan")
    avatar = relationship("ProfileAvatar", back_populates="profile", uselist=False, cascade="all, delete-orphan")
    specialisms = relationship("ProfileSpecialism", back_populates="profile", foreign_keys="ProfileSpecialism.profile_id", cascade="all, delete-orphan")
...
class IdentityProvider(ProfileBase):
    ...
    idp_name = Column(Text, nullable=False, unique=True)  # e.g., 'google', 'microsoft', 'local'
...
class ProfileIdentity(ProfileBase):
    ...
    profile_identity_id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenant.tenant_id", ondelete="CASCADE"), nullable=False)
    profile_id = Column(UUID(as_uuid=True), ForeignKey("profile.profile_id", ondelete="CASCADE"), nullable=False)
    idp_id = Column(SmallInteger, ForeignKey("identity_provider.idp_id", ondelete="CASCADE"), nullable=False)
    ...
    __table_args__ = (
        ...
        Index("ix_profile_identity_idp_subject", "idp_id", "idp_tenant_subject", unique=True),
    )
...
class ProfileDisplay(ProfileBase):
    ...
    profile_id = Column(UUID(as_uuid=True), ForeignKey("profile.profile_id", ondelete="CASCADE"), primary_key=True)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenant.tenant_id", ondelete="CASCADE"), nullable=False)
...
class ProfileAvatar(ProfileBase):
    ...
    profile_id = Column(UUID(as_uuid=True), ForeignKey("profile.profile_id", ondelete="CASCADE"), primary_key=True)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenant.tenant_id", ondelete="CASCADE"), nullable=False)
    avatar_source = Column(Text, nullable=False, default="preset")  # 'preset' or 'uploaded'
    avatar_preset_id = Column(UUID(as_uuid=True), ForeignKey("avatar_preset.avatar_preset_id", ondelete="SET NULL"), nullable=True)
...
class ProfileSpecialism(ProfileBase):
    ...
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenant.tenant_id", ondelete="CASCADE"), primary_key=True)
    profile_id = Column(UUID(as_uuid=True), ForeignKey("profile.profile_id", ondelete="CASCADE"), primary_key=True)
    specialism_id = Column(UUID(as_uuid=True), ForeignKey("specialism.specialism_id", ondelete="CASCADE"), primary_key=True)
    proficiency_level = Column(Text, nullable=True)  # e.g., 'beginner', 'intermediate', 'expert'
    ...
    assigned_by_profile_id = Column(UUID(as_uuid=True), ForeignKey("profile.profile_id", ondelete="SET NULL"), nullable=True)
    
    # Relationships
    profile = relationship("Profile", back_populates="specialisms", foreign_keys=[profile_id])
