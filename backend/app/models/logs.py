...
class RequestTrace(LogBase):
    ...
    request_id = Column(String(100), primary_key=True, nullable=False) # trcae ID across logs
    first_seen_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False) # when the request was first captured
    last_seen_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False) # most recent log event for this request, gets updated with each new log entry for this request
    source = Column(String(50), nullable=True) # originating service or component that initiated the request
...
class ApplicationLog(LogBase):
    ...
    source = Column(String(50), nullable=False, index=True) # originating service or component that generated the log
    subsystem = Column(String(100), nullable=False, index=True) # specific subsystem or module within the source that generated the log
    action = Column(String(200), nullable=False, index=True) # specific action or event being logged, useful for categorization and filtering
    logger_name = Column(String(150), nullable=True, index=True) # trace the event back to the exact code logger that produced it
    ...
    autotask_ticket_id = Column(Integer, nullable=True, index=True) # in case log is related to a certain ticket
...
class ErrorLog(LogBase):
    ...
    severity = Column(String(20), nullable=False, index=True) # i.e. critical, high, medium, low
    error_resolution = Column(Text, nullable=True) # error resolution details, can be updated after the error is resolved for future reference
    details = Column(JSONB, nullable=True)
...
class UIClickAnalyticsLog(LogBase):
    ...
    environment = Column(String(50), nullable=True, index=True)
    page_path = Column(String(255), nullable=True, index=True) # i.e. 'Dashboard', 'TicketDetails' etc.
    component = Column(String(100), nullable=False, index=True) # i.e. 'ticket-card', 'tickets-list-container' etc.
    action_type = Column(String(100), nullable=False, index=True) # action type i.e. 'view', 'edit' , 'reassign category' etc.
    element_id = Column(String(150), nullable=True, index=True) # specific element ID in the UI, useful for tracing interactions with specific UI elements
    duration_ms = Column(Numeric(10, 2), nullable=True) # time take for intercation if applicable
    details = Column(JSONB, nullable=True)
