...
class ProfileRepository:
    ...
    async def get_profile_by_id(
        ...
        query = (
            ...
            .where(
                and_(Profile.profile_id == profile_id, Profile.tenant_id == tenant_id)
            )
        )
    ...
    async def get_profiles_by_tenant(
        ...
        query = (
            select(Profile)
            .options(selectinload(Profile.display), selectinload(Profile.avatar))
    ...
    async def get_profiles_by_tenant_with_specialisms(
        ...
        query = (
            ...
            .options(
                selectinload(Profile.display),
                selectinload(Profile.specialisms).selectinload(
                    ProfileSpecialism.specialism
                ),
            )
            .where(Profile.tenant_id == tenant_id)
        )
    ...
    async def update_profile(
        self, profile_id: UUID, tenant_id: UUID, **updates
    ) -> Optional[Profile]:
    ...
    async def update_display_name(
        self, profile_id: UUID, tenant_id: UUID, display_name: str
    ) -> bool:
    ...
    async def get_profiles_by_display_names(
        ...
        normalized_names = [
            name.strip().lower() for name in display_names if name and name.strip()
        ]
...
class IdentityRepository:
    ...
    async def get_identity_provider_by_name(
        self, idp_name: str
    ...
    async def link_profile_identity(
        self, profile_id: UUID, tenant_id: UUID, idp_id: int, idp_tenant_subject: str
    ) -> ProfileIdentity:
    ...
    async def get_profile_by_identity(
        self, idp_id: int, idp_tenant_subject: str
    ) -> Optional[Profile]:
    ...
    async def update_last_login(self, idp_id: int, idp_tenant_subject: str) -> bool:
...
class SpecialismRepository:
    ...
    async def get_specialisms_by_tenant(
        ...
        if active_only:
            query = query.where(Specialism.is_active)
