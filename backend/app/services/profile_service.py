...
class ProfileService:
    ...
    async def update_profile(
        self, profile_id: UUID, tenant_id: UUID, profile_data: ProfileUpdate
    ) -> Optional[ProfileResponse]:
        ...
        if "display_name" in updates:
            ...
            await self.profile_repo.update_display_name(
                profile_id, tenant_id, display_name
            )

        # Update remaining fields
        if updates:
            profile = await self.profile_repo.update_profile(
                profile_id, tenant_id, **updates
            )
            if not profile:
                return None
    ...
    async def deactivate_profile(
        self, profile_id: UUID, tenant_id: UUID, reason: Optional[str] = None
    ) -> bool:
    ...
    async def search_profiles(
        ...
        profiles = await self.profile_repo.search_profiles_by_name(
            tenant_id=tenant_id, search_term=search_term, limit=limit
        )
        return [ProfileResponse.model_validate(p) for p in profiles]

    async def link_external_identity(
        self, identity_data: ProfileIdentityCreate
    ) -> bool:
    ...
    async def find_profile_by_identity(
        self, idp_name: str, idp_tenant_subject: str
    ) -> Optional[ProfileResponse]:
        ...
        profile = await self.identity_repo.get_profile_by_identity(
            idp_id=idp.idp_id, idp_tenant_subject=idp_tenant_subject
        )
    ...
    async def resolve_entra_profile(
        ...
        tenant = await self.tenant_repo.get_tenant_by_name(
            settings.ENTRA_INTERNAL_TENANT_NAME
        )
        if not tenant:
        ...
        idp = await self.identity_repo.get_or_create_identity_provider(
            settings.ENTRA_IDP_NAME
        )
        ...
        if profile:
            ...
            refreshed = await self.profile_repo.get_profile_by_id(
                profile.profile_id, tenant.tenant_id
            )
        ...
        await self.identity_repo.update_last_login(idp.idp_id, subject)
        resolved = await self.profile_repo.get_profile_by_id(
            created_profile.profile_id, tenant.tenant_id
        )
...
class SpecialismService:
    ...
    async def create_specialism(
        self, specialism_data: SpecialismCreate
    ) -> SpecialismResponse:
    ...
    async def replace_profile_specialisms_from_category_keys(
        ...
        if unknown_keys:
            raise ValueError(
                f"Unsupported specialism keys: {', '.join(sorted(unknown_keys))}"
            )
        ...
        existing_by_key = {
            specialism.specialism_key: specialism for specialism in existing_specialisms
        }
